...
try:
    ...
    print( sys.exc_info() )
    print( f"Module 'pystac.extensions.archive.ArchiveExtension' import error in {__file__}" )

try:
    import utils_archive_extension
except:
    print( sys.exc_info() )
    print( f"Module 'utils_archive_extension' import error in {__file__}" )

try:
    from utils_archive_extension import variableDict, zarrStoreList, \
...
tarStoreList = [ zarr.storage.TarStore( storePath, mode='r') for storePath in tarStorePathList ]
...
tape_assetList = [pystac.asset.Asset.from_dict(tape_asset) for tape_asset in tape_assets]
